...
export class ColorService {
  ...
  reserveColor(reservations: Map<string, { color: string; playerId: string; expiresAt: number }>, playerId: string, ttlMs: number): { reservationId: string; color: string; expiresAt: number } {
  ...
  private pickColor(): string {
    for (const c of BASE_COLORS) {
      if (!this.activeColors.has(c)) return c;
    }

    for (let i = 0; i < 360; i += 17) {
      const candidate = `hsl(${i} 100% 55%)`;
      if (!this.activeColors.has(candidate)) return candidate;
    }
